...
## ⚠️ Before switching machines: commit and push the app repos too

These docs describe substantial code changes already made in `ecommerce-back-office` and
...
orders — see `ECOMMERCE_TASK_BREAKDOWN.md` for the full list of what's done). **As of this
docs snapshot, that work sits uncommitted in both repos.** Cloning them fresh on another
machine gets you the old, unmodified code — none of the work described here — unless it was
committed and pushed first. If you're reading this after a pull on a new machine and things
look thinner than the task breakdown describes, that's why: go back to the original machine,
commit, and push.
...
## Gotcha found while moving these docs

`scripts/gen-schema-meta.mjs`'s default source path (in both `ecommerce-back-office` and
`ecommerce-consumer`) was updated to point at this repo's `ECOMMERCE_INVENTORY_SCHEMAS.json`
(it used to live at the old shared workspace root — see git history on that one-line change).
**Verifying that fix surfaced a separate, pre-existing problem, unrelated to the move:**
running the regenerator actually deletes `isComplexFieldType`/`PRIMITIVE_FIELD_TYPES` from the
committed `schema-meta.ts` in both apps — that function is hand-maintained directly in the
"AUTO-GENERATED — do not hand-edit" file, not actually produced by the generator script, and
is depended on elsewhere (`collections.ts` imports it). **Do not run
`node scripts/gen-schema-meta.mjs` in either app without checking the diff first** — if it
proposes removing that function, restore it (`git checkout -- src/lib/blocks/schema-meta.ts`)
rather than committing the regenerated output as-is. The generator script itself should
probably be taught to emit that helper, so this stops being a trap — not done here, out of
scope for a docs-consolidation pass.

## Where to pick up next

Check the unchecked boxes in `ECOMMERCE_TASK_BREAKDOWN.md`, roughly in this order as of this
snapshot:

1. Import `P0_POLICY_FIXES.json` and `COMMERCE_SCHEMAS_DRAFT.json` via your own `blocks` CLI
   session (see each file's `.md` for exact steps) — several already-built features are
   inert until this happens.
2. The shared compare-and-swap reserve/commit/release module
   (`ECOMMERCE_PLATFORM_ON_BLOCKS.md` §5.1) — the actual oversell-prevention mechanism.
3. Backoffice Commerce admin screens (Orders) — needs the schema import first.
4. Remaining Phase 2-4 items: bins/zones, lots/serials, costing, replenishment, allocation
   strategy — see `ECOMMERCE_TASK_BREAKDOWN.md` §4-6.
